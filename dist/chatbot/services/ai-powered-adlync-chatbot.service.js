...
var AIPoweredAdlyncChatbotService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.AIPoweredAdlyncChatbotService = void 0;
const common_1 = require("@nestjs/common");
const config_1 = require("@nestjs/config");
const uuid_1 = require("uuid");
const openai_1 = require("openai");
let AIPoweredAdlyncChatbotService = AIPoweredAdlyncChatbotService_1 = class AIPoweredAdlyncChatbotService {
    constructor(configService) {
        this.configService = configService;
        this.logger = new common_1.Logger(AIPoweredAdlyncChatbotService_1.name);
...
`;
    ...
    async processMessage(message, sessionId, userId) {
        try {
            ...
            const chatResponse = {
                ...
                suggestedActions: this.generateSuggestedActions(message, response.isFromKnowledgeBase),
                timestamp: new Date(),
            };
            this.logger.log(`AI-powered response generated for session ${session.id}`);
            return chatResponse;
        }
        catch (error) {
            this.logger.error('Error processing message:', error);
            return {
                message: 'I apologize, but I encountered an error. Please try again or contact our Adlync Solutions team directly for assistance!',
    ...
    async generateAIResponse(message, session) {
        if (!this.openai) {
...
Remember: Be concise, include numbers, avoid promises, encourage consultation.`;
    ...
    generateFallbackResponse(message) {
        ...
        if (messageLower.includes('adlync') ||
    ...
    generateSuggestedActions(query, isFromKnowledgeBase) {
        const suggestions = [];
    ...
    cleanupExpiredSessions() {
        ...
        for (const [sessionId, session] of this.sessions.entries()) {
            if (now.getTime() - session.lastActivity.getTime() >
                this.sessionTimeout) {
    ...
    async initializeKnowledgeBase(serviceData) {
        this.logger.log('AI-powered Adlync Solutions chatbot initialized with OpenAI integration');
...
exports.AIPoweredAdlyncChatbotService = AIPoweredAdlyncChatbotService;
exports.AIPoweredAdlyncChatbotService = AIPoweredAdlyncChatbotService = AIPoweredAdlyncChatbotService_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [config_1.ConfigService])
], AIPoweredAdlyncChatbotService);
//# sourceMappingURL=ai-powered-adlync-chatbot.service.js.map