...
let VapiService = VapiService_1 = class VapiService {
    constructor(config, httpService) {
        this.config = config;
        this.httpService = httpService;
        this.logger = new common_1.Logger(VapiService_1.name);
        this.privateKey = this.config.get("VAPI_PRIVATE_KEY");
        this.publicKey = this.config.get("VAPI_PUBLIC_KEY");
        this.baseUrl = this.config.get("VAPI_BASE_URL");
    ...
    validateConfiguration() {
        if (!this.privateKey || !this.publicKey) {
            throw new Error("Missing required Vapi configuration: privateKey or publicKey");
        }
        this.logger.log("🎙️ Vapi Configuration Status:");
        this.logger.log(`✅ Private Key: ${this.privateKey ? "Configured" : "Missing"}`);
        this.logger.log(`✅ Public Key: ${this.publicKey ? "Configured" : "Missing"}`);
    }
...
"Thank you! Aapka din bahut hi accha rahe! 😊"`,
                ...
                firstMessage: "Hello! Namaste! I'm from Adlync Solutions. How can I help you today?",
            ...
            const response = await (0, rxjs_1.firstValueFrom)(this.httpService.post(`${this.baseUrl}/assistant`, assistantConfig, {
                headers: {
            ...
            this.logger.log(`✅ Assistant created successfully: ${response.data?.id}`);
            return {
                success: true,
    ...
    async initiateCall(callRequest) {
        try {
            ...
            console.log("🔍 DEBUG - Vapi Private Key:", this.privateKey ? "Present" : "Missing");
            const response = await (0, rxjs_1.firstValueFrom)(this.httpService.post(`${this.baseUrl}/call`, payload, {
                headers: {
                    Authorization: `Bearer ${this.privateKey}`,
                    "Content-Type": "application/json",
                },
            ...
            this.logger.log(`✅ Vapi call initiated successfully: ${response.data?.id}`);
            return {
                success: true,
    ...
    async getCallStatus(callId) {
        try {
            const response = await (0, rxjs_1.firstValueFrom)(this.httpService.get(`${this.baseUrl}/call/${callId}`, {
                headers: {
                    Authorization: `Bearer ${this.privateKey}`,
                },
                timeout: 10000,
            ...
            return this.handleError(error, `Failed to get call status for: ${callId}`);
        }
    }
    async listAssistants() {
        try {
            const response = await (0, rxjs_1.firstValueFrom)(this.httpService.get(`${this.baseUrl}/assistant`, {
                headers: {
                    Authorization: `Bearer ${this.privateKey}`,
                },
                timeout: 10000,
    ...
    async listPhoneNumbers() {
        try {
            const response = await (0, rxjs_1.firstValueFrom)(this.httpService.get(`${this.baseUrl}/phone-number`, {
                headers: {
                    Authorization: `Bearer ${this.privateKey}`,
                },
                timeout: 10000,
...
exports.VapiService = VapiService = VapiService_1 = __decorate([
